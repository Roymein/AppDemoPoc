...
import com.dev.client.BuildConfig
import com.dev.client.Options
import com.dev.client.R
import com.dev.client.extension.isTablet
...
class SettingsFragment : PreferenceFragmentCompat(),
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        pipRatioPref?.let {
            ...
            pipRatioPref.title =
                "${getString(R.string.preference_pip_ratio_title)}\n(${ratioArray[index]})"
        }

        val playerNextActionPref: Preference? =
            findPreference(getString(R.string.preference_next_action_key))
        playerNextActionPref?.let {
            ...
            playerNextActionPref.title =
                "${getString(R.string.preference_next_action_title)}\n(${nextActionArray[action.value]})"
        }
        ...
        shareUrlPref?.let {
            ...
            shareUrlPref.summary =
                if (schemeUrl.isNullOrEmpty()) getString(R.string.preference_share_url_summary) else schemeUrl
        }

        val loadingHexPref: Preference? =
            findPreference(getString(R.string.preference_loading_progress_key))
        loadingHexPref?.let {
            val hex = Options.loadingProgressColor()
            loadingHexPref.summary =
                if (hex.isNullOrEmpty()) getString(R.string.preference_loading_progress_summary) else hex
        }

        val tabletAspectPref =
            findPreference<SwitchPreferenceCompat>(getString(R.string.preference_tablet_aspect_key))
    ...
    override fun onResume() {
        ...
        if (ContextCompat.checkSelfPermission(
            ...
            val callPref =
                findPreference<SwitchPreferenceCompat>(getString(R.string.preference_call_key))
            callPref?.isChecked = true
        }

        // 방송 자동종료 옵션(기본값 true)
        val autoClosePref =
            findPreference<SwitchPreferenceCompat>(getString(R.string.preference_auto_close_key))
        ...
        when (key) {
            getString(R.string.preference_mute_key) -> {
                sharedPreferences?.let {
                    ...
                    if (!isKeepPlayVideo) {
                        val mutePref =
                            findPreference<SwitchPreferenceCompat>(getString(R.string.preference_mute_key))
                        mutePref?.isChecked = false
                        Toast.makeText(
                            requireContext(),
                            getString(R.string.toast_headset_mute_option),
                            Toast.LENGTH_SHORT
                        ).show()
                        return
                    }
    ...
    private fun showPipRatioDialog() {
        ...
        builder.setItems(ratioTextArray) { _, which ->
            ...
            pipPref?.let {
                pipPref.title =
                    "${getString(R.string.preference_pip_ratio_title)}(${ratioTextArray[which]})"
    ...
    private fun showShareUrlInputDialog() {
        val inflater =
            requireContext().getSystemService(Application.LAYOUT_INFLATER_SERVICE) as LayoutInflater
        ...
        builder.setPositiveButton(getString(R.string.dialog_confirm)) { dialog, _ ->
            val schemeUrlPref: Preference? =
                findPreference(getString(R.string.preference_share_url_key))
            schemeUrlPref?.let {
    ...
    private fun showLoadingProgressColorInputDialog() {
        ...
        builder.setPositiveButton(getString(R.string.dialog_confirm)) { dialog, _ ->
            val loadingProgressColorPref: Preference? =
                findPreference(getString(R.string.preference_loading_progress_key))
            loadingProgressColorPref?.let {
                val hexColor = etInput.text.toString()
                if (hexColor.isEmpty()) {
                    loadingProgressColorPref.summary =
                        getString(R.string.preference_loading_progress_summary)
                } else {
    ...
    private fun showPlayerNextActionDialog() {
        ...
        builder.setItems(nextActionTextArray) { _, which ->
            val nextActonPref: Preference? =
                findPreference(getString(R.string.preference_next_action_key))
            nextActonPref?.let {
                nextActonPref.title =
                    "${getString(R.string.preference_next_action_title)} (${nextActionTextArray[which]})"
